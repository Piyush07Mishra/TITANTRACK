...
class Rental(models.Model):
    ...
    # ----------------------
    checkout_lat = models.FloatField(blank=True, null=True, help_text="Latitude at checkout")
    checkout_lng = models.FloatField(blank=True, null=True, help_text="Longitude at checkout")
    ...
    week = models.CharField(max_length=10, help_text="Year-week (e.g., 2025-W35)", default=default_rental_week)
...
class Operator(models.Model):
    ...
    site_location = models.CharField(max_length=100, blank=True, null=True)
...
# --- Helper function for generating secure passwords ---
def generate_secure_password(length=10):
    alphabet = string.ascii_letters + string.digits + "!@#$%"
    return ''.join(secrets.choice(alphabet) for _ in range(length))


# Guard flag to prevent signal re-entry
_creating_operator_user = False


# --- Signal: auto-create User account when an Operator is created ---
@receiver(post_save, sender=Operator)
def create_user_for_operator(sender, instance, created, **kwargs):
    global _creating_operator_user

    # Only run on fresh creation AND when no user is linked yet
    if not created or instance.user_id is not None:
        return

    # Prevent re-entry
    if _creating_operator_user:
        return

    _creating_operator_user = True
    try:
        # Build unique username from operator_id
        username = instance.operator_id
        if User.objects.filter(username=username).exists():
            username = f"{username}_{instance.pk}"

        raw_password = generate_secure_password()

        # Create Django User (this will trigger User post_save signals)
        ...
        # Create UserProfile with operator role
        UserProfile.objects.update_or_create(
            user=user,
            defaults={'role': 'operator'}
        )

        # Link user back to Operator record (using .update to avoid re-triggering this signal)
        Operator.objects.filter(pk=instance.pk).update(
            user=user,
...
Your CatRent operator account has been created by the admin.

You can log in at: {settings.SITE_URL}/login/
...
""",
            ...
            # Log email failure but do not crash the save
            print(f"[CatRent] Email send failed for {instance.email}: {e}")
    finally:
        _creating_operator_user = False
...
def create_user_profile(sender, instance, created, **kwargs):
    """Create a UserProfile when a new User is created (only on creation)."""
    if created:
