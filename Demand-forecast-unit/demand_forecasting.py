...
for i, eq_type in enumerate(equipment_types, 1):
    ...
    plt.title(f"{eq_type} - 2025 Weekly Demand", fontsize=14)
...
plt.tight_layout()
plt.savefig("equipment_type_demand_forecast_2025.png")
print("Combined visualization saved as 'equipment_type_demand_forecast_2025.png'")
...
for eq_type in equipment_types:
    ...
    plt.title(f"{eq_type} - 2025 Weekly Demand Forecast", fontsize=16)
    plt.xlabel("Week of 2025", fontsize=12)
    ...
    plt.tight_layout()
    plt.savefig(f"{eq_type}_demand_forecast_2025.png")
    print(f"Saved {eq_type}_demand_forecast_2025.png")
...
def sync_outputs_to_dashboard_forecast():
    """Clean old forecast images in catrent/forecast and copy the latest generated outputs."""
    script_dir = Path(__file__).resolve().parent
    forecast_dir = script_dir.parent / "catrent" / "forecast"

    if not forecast_dir.exists():
        print(f"Warning: Dashboard forecast folder not found: {forecast_dir}")
        return

    # Remove stale dashboard artifacts before copying fresh ones.
    cleanup_patterns = [
        "*_demand_forecast_2025.png",
        "equipment_demand_forecast_2025_combined.png",
        "feature_importance.png",
        "new_feature_importance.png",
    ]
    removed = 0
    for pattern in cleanup_patterns:
        for old_file in forecast_dir.glob(pattern):
            old_file.unlink(missing_ok=True)
            removed += 1

    files_to_move = [
        "Bulldozer_demand_forecast_2025.png",
        "Compactor_demand_forecast_2025.png",
        "Crane_demand_forecast_2025.png",
        "Excavator_demand_forecast_2025.png",
        "Loader_demand_forecast_2025.png",
        "feature_importance.png",
        "new_feature_importance.png",
    ]
    ...
    for file_name in files_to_move:
        ...
        if src.exists():
            shutil.move(str(src), str(forecast_dir / file_name))
            moved += 1

    # Dashboard fallback expects this exact combined file name.
    combined_src = script_dir / "equipment_type_demand_forecast_2025.png"
    if combined_src.exists():
        shutil.move(str(combined_src), str(forecast_dir / "equipment_demand_forecast_2025_combined.png"))
        moved += 1

    print(f"Synced forecast assets to dashboard folder: removed={removed}, moved={moved}")


sync_outputs_to_dashboard_forecast()
